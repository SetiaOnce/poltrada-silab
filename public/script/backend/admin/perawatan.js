...
jQuery(document).ready(function() {
    ...
    /*************************
        OnChange Fid alat for Form Perawatan Alat
    *************************/
...
const _loadDataPerawatan = () => {
    table = $('#dt-perawatanAlat').DataTable({
        ...
        serverSide: true,
        ajax: {
            "url" : BASE_URL+ "/app_admin/ajax/load_perawatan",
...
const _addData = () => {
    ...
    $("#card-form .card-header .card-title").html(
        `<h3 class="fw-bolder fs-2 text-gray-900"><i class="bi bi-window-plus fs-2 text-gray-900 me-2"></i>Form Tambah Perawatan Alat Peraga</h3>`
    ),
...
const _editData = (idp) => {
    ...
    //Ajax load from ajax
    $.ajax({
        url: BASE_URL+ '/app_admin/ajax/perawatan_edit',
        headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
        type: 'GET',
        dataType: 'JSON',
        data: {
        ...
        success: function (data) {
            ...
            if (data.status == true) {
                ...
                // tanggal perawatan
                $("#tgl_perawatan").flatpickr({
                    defaultDate: data.tgl_perawatan,
                    dateFormat: "d/m/Y"
                });
                //fid alat peraga
                ...
                $("#card-form .card-header .card-title").html(
                    `<h3 class="fw-bolder fs-2 text-gray-900"><i class="bi bi-pencil-square fs-2 text-gray-900 me-2"></i>Form Edit Data Perawatan Alat</h3>`
                ),
                $("#card-data").hide(), $("#card-form").show();
...
$("#btn-save").on("click", function (e) {
    ...
    if (fid_alat_peraga.val() == '') {
        ...
        toastr.error('Tanggal perawatan masih kosong...', 'Uuppss!', {"progressBar": true, "timeOut": 1500});
        tgl_perawatan.focus();
        $('#btn-save').html('<i class="far fa-save"></i> Simpan').attr('disabled', false);
        return false;
    }if (preview_foto == '') {
    ...
    Swal.fire({
        ...
        if (result.value) {
            let target = document.querySelector("#card-form"), blockUi = new KTBlockUI(target, { message: messageBlockUi });
            blockUi.block(), blockUi.destroy();
            let formData = new FormData($("#form-data")[0]), ajax_url = BASE_URL+ "/app_admin/ajax/perawatan_save";
            if(save_method == 'update_data') {
                ajax_url = BASE_URL+ "/app_admin/ajax/perawatan_update";
            }
            $.ajax({
                url: ajax_url,
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                ...
                success: function (data) {
                    ...
                    if (data.status == true) {
                        var message = 'Perawatan berhasil diperbarui'
                        if(save_method == 'add_data'){
                            var message = 'Perawatan baru berhasil ditambahkan'
                        }
                        Swal.fire({
                            title: "Success!",
                            text: message,
...
const _deleteData = (idp) => {
    Swal.fire({
        ...
        if (result.value) {
            ...
            // Load Ajax
            $.ajax({
                url: BASE_URL+ "/app_admin/ajax/perawatan_destroy",
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                type: "POST",
                dataType: "JSON",
                data: {
                    ...
                    blockUi.release(), blockUi.destroy();
                    Swal.fire({ title: "Success!", html: 'Data perawatan berhasil dihapus...', icon: "success", allowOutsideClick: false }).then(function (result) {
                        _loadDataPerawatan();
                    });
                }, error: function (jqXHR, textStatus, errorThrown) {
                    blockUi.release(), blockUi.destroy();
