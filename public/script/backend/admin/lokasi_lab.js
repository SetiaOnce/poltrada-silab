...
var table;
//Load Datatables
const _loadDataLokasiLaboratorium = () => {
    table = $('#dt-lokasiLab').DataTable({
        searchDelay: 300,
        processing: true,
        serverSide: true,
        ajax: {
            "url" : BASE_URL+ "/app_admin/ajax/load_lokasi_laboratorium",
        ...
        fnDrawCallback: function (settings, display) {
            ...
            //Custom Table
            $("#dt-lokasiLab_length select").selectpicker(),
...
const _addData = () => {
    ...
    $("#card-form .card-header .card-title").html(
        `<h3 class="fw-bolder fs-2 text-gray-900"><i class="bi bi-window-plus fs-2 text-gray-900 me-2"></i>Form Tambah Lokasi Laboratorium</h3>`
    ),
...
const _editData = (idp) => {
    ...
    //Ajax load from ajax
    $.ajax({
        url: BASE_URL+ '/app_admin/ajax/lokasi_laboratorium_edit',
        headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
        type: 'GET',
        dataType: 'JSON',
        data: {
        ...
        success: function (data) {
            ...
            if (data.status == true) {
                ...
                $('#nama_lokasi').val(data.row.nama_lokasi);
                ...
                $('#iGroup-Status').show();
                $("#card-form .card-header .card-title").html(
                    `<h3 class="fw-bolder fs-2 text-gray-900"><i class="bi bi-pencil-square fs-2 text-gray-900 me-2"></i>Form Edit Lokasi Laboratorium</h3>`
                ),
                $("#card-data").hide(), $("#card-form").show();
            } else {
...
$("#btn-save").on("click", function (e) {
    ...
    var nama_lokasi = $('#nama_lokasi');
    var alamat = $('#alamat');
    
    if (nama_lokasi.val() == '') {
        toastr.error('Nama lokasi masih kosong...', 'Uuppss!', {"progressBar": true, "timeOut": 1500});
        nama_lokasi.focus();
    ...
    Swal.fire({
        ...
        if (result.value) {
            let target = document.querySelector("#card-form"), blockUi = new KTBlockUI(target, { message: messageBlockUi });
            blockUi.block(), blockUi.destroy();
            let formData = new FormData($("#form-data")[0]), ajax_url = BASE_URL+ "/app_admin/ajax/lokasi_laboratorium_save";
            if(save_method == 'update_data') {
                ajax_url = BASE_URL+ "/app_admin/ajax/lokasi_laboratorium_update";
            }
            $.ajax({
                url: ajax_url,
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                ...
                success: function (data) {
                    ...
                    if (data.status == true) {
                        var message = 'Lokasi laboratorium berhasil diperbarui'
                        if(save_method == 'add_data'){
                            var message = 'Lokasi laboratorium baru berhasil ditambahkan'
                        }
                        Swal.fire({title: "Success!",text: message,icon: "success",allowOutsideClick: false,
                        }).then(function (result) {
                            _closeCard('form_lokasi_lab'), _loadDataLokasiLaboratorium();
                        });
                    } else {
...
const _updateStatus = (idp, value) => {
    ...
    let textSwal = textLbl+ ' lokasi laboratorium sekarang ?';
    Swal.fire({
        title: "",
        html: textSwal,
        icon: "question",
        ...
        if (result.value) {
            ...
            // Load Ajax
            $.ajax({
                url: BASE_URL+ "/app_admin/ajax/lokasi_laboratorium_update_status",
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                type: "POST",
                dataType: "JSON",
                data: {
