...
const _loadDataBanner = () => {
    table = $('#dt-banners').DataTable({
        searchDelay: 300,
        processing: true,
        serverSide: true,
        ajax: {
            "url" : BASE_URL+ "/app_admin/ajax/load_banner",
        ...
        columnDefs: [
            ...
            { "width": "60%", "targets": 1, "className": "align-top text-center" },
        ...
        fnDrawCallback: function (settings, display) {
            ...
            //Custom Table
            $("#dt-banners_length select").selectpicker(),
...
const _clearForm = () => {
    if (save_method == "" || save_method == "add_data") {
        $("#form-data")[0].reset(), _loadDropifyFile('', '#file_image');
...
const _addData = () => {
    ...
    $("#card-form .card-header .card-title").html(
        `<h3 class="fw-bolder fs-2 text-gray-900"><i class="bi bi-window-plus fs-2 text-gray-900 me-2"></i>Form Tambah Banner</h3>`
    ),
...
const _editSlide = (idp) => {
    ...
    //Ajax load from ajax
    $.ajax({
        url: BASE_URL+ '/app_admin/ajax/banner_edit',
        headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
        type: 'GET',
        dataType: 'JSON',
        data: {
        ...
        success: function (data) {
            ...
            if (data.status == true) {
                ...
                _loadDropifyFile(data.file_image, '#file_image');
                //Status
                ...
                $("#card-form .card-header .card-title").html(
                    `<h3 class="fw-bolder fs-2 text-gray-900"><i class="bi bi-pencil-square fs-2 text-gray-900 me-2"></i>Form Edit Banner</h3>`
                ),
                $("#card-data").hide(), $("#card-form").show();
            } else {
...
$("#btn-save").on("click", function (e) {
    ...
    priview_filebanner = $('#iGroup-file_image .dropify-preview .dropify-render').html();
    
    if (priview_filebanner == '') {
        toastr.error('File banner masih kosong...', 'Uuppss!', {"progressBar": true, "timeOut": 1500});
        $('#iGroup-file_image .dropify-wrapper').addClass('border-2 border-danger').stop().delay(1500).queue(function () {
    ...
    Swal.fire({
        ...
        if (result.value) {
            let target = document.querySelector("#card-form"), blockUi = new KTBlockUI(target, { message: messageBlockUi });
            blockUi.block(), blockUi.destroy();
            let formData = new FormData($("#form-data")[0]), ajax_url = BASE_URL+ "/app_admin/ajax/banner_save";
            if(save_method == 'update_data') {
                ajax_url = BASE_URL+ "/app_admin/ajax/banner_update";
            }
            $.ajax({
                url: ajax_url,
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                ...
                success: function (data) {
                    ...
                    if (data.status == true) {
                        var message = 'Konten banner berhasil perbarui'
                        if(save_method == 'add_data'){
                            var message = 'Konten banner baru berhasil ditambahkan'
                        }
                        Swal.fire({
                            title: "Success!",
                            text: message,
                            ...
                            _closeCard('form_slide'), _loadDataBanner();
                        });
                    } else {
...
const _updateStatus = (idp, value) => {
    ...
    let textSwal = textLbl+ ' Banner sekarang ?';
    Swal.fire({
        title: "",
        html: textSwal,
        icon: "question",
        ...
        if (result.value) {
            ...
            // Load Ajax
            $.ajax({
                url: BASE_URL+ "/app_admin/ajax/banner_update_status",
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                type: "POST",
                dataType: "JSON",
                data: {
...
const _deleteSlide = (idp) => {
    Swal.fire({
        ...
        if (result.value) {
            ...
            // Load Ajax
            $.ajax({
                url: BASE_URL+ "/app_admin/ajax/banner_destroy",
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                type: "POST",
                dataType: "JSON",
                data: {
