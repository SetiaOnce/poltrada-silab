...
const _loadDataFaq = () => {
    table = $('#dt-faqs').DataTable({
        searchDelay: 300,
        processing: true,
        serverSide: true,
        ajax: {
            "url" : BASE_URL+ "/app_admin/ajax/load_faq",
        ...
        fnDrawCallback: function (settings, display) {
            ...
            //Custom Table
            $("#dt-faqs_length select").selectpicker(),
...
const _clearForm = () => {
    if (save_method == "" || save_method == "add_data") {
        $("#form-data")[0].reset(), $('#isi').summernote('code', '');
...
const _addData = () => {
    ...
    $("#card-form .card-header .card-title").html(
        `<h3 class="fw-bolder fs-2 text-gray-900"><i class="bi bi-window-plus fs-2 text-gray-900 me-2"></i>Form Tambah FAQ</h3>`
    ),
...
const _editData = (idp) => {
    ...
    //Ajax load from ajax
    $.ajax({
        url: BASE_URL+ '/app_admin/ajax/faq_edit',
        headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
        type: 'GET',
        dataType: 'JSON',
        data: {
        ...
        success: function (data) {
            ...
            if (data.status == true) {
                ...
                $('#isi').summernote('code', data.row.isi);
                //Status
                ...
                $('#iGroup-Status').show();
                $("#card-form .card-header .card-title").html(
                    `<h3 class="fw-bolder fs-2 text-gray-900"><i class="bi bi-pencil-square fs-2 text-gray-900 me-2"></i>Form Edit FAQ</h3>`
                ),
                $("#card-data").hide(), $("#card-form").show();
            } else {
...
$("#btn-save").on("click", function (e) {
    ...
    if (isi.summernote('isEmpty')) {
        toastr.error('Isi masih kosong...', 'Uuppss!', {"progressBar": true, "timeOut": 1500});
        isi.focus();
        $('#btn-save').html('<i class="far fa-save"></i> Simpan').attr('disabled', false);
        _blockUiPages(0);
        return false;
    ...
    Swal.fire({
        ...
        if (result.value) {
            let target = document.querySelector("#card-form"), blockUi = new KTBlockUI(target, { message: messageBlockUi });
            blockUi.block(), blockUi.destroy();
            let formData = new FormData($("#form-data")[0]), ajax_url = BASE_URL+ "/app_admin/ajax/faq_save";
            if(save_method == 'update_data') {
                ajax_url = BASE_URL+ "/app_admin/ajax/faq_update";
            }
            $.ajax({
                url: ajax_url,
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                ...
                success: function (data) {
                    ...
                    if (data.status == true) {
                        var message = 'FAQ berhasil perbarui'
                        if(save_method == 'add_data'){
                            var message = 'FAQ baru berhasil ditambahkan'
                        }
                        Swal.fire({
                            title: "Success!",
                            text: message,
                            ...
                            _closeCard('form_faq'), _loadDataFaq();
                        });
                    } else {
...
const _updateStatus = (idp, value) => {
    ...
    let textSwal = textLbl+ ' Faq sekarang ?';
    Swal.fire({
        title: "",
        html: textSwal,
        icon: "question",
        ...
        if (result.value) {
            ...
            // Load Ajax
            $.ajax({
                url: BASE_URL+ "/app_admin/ajax/faq_update_status",
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                type: "POST",
                dataType: "JSON",
                data: {
...
const _deleteData = (idp) => {
    Swal.fire({
        ...
        if (result.value) {
            ...
            // Load Ajax
            $.ajax({
                url: BASE_URL+ "/app_admin/ajax/faq_destroy",
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                type: "POST",
                dataType: "JSON",
                data: {
